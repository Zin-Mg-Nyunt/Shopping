<script setup></script>
<template>
    <footer
        class="bg-card border-t"
        role="contentinfo"
        aria-label="Site footer"
    >
        <div
            class="container mx-auto px-4 py-12 sm:px-6 lg:px-8"
        >
            <div
                class="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4"
            >
                <!-- About Us -->
                <div>
                    <h3
                        class="mb-4 text-lg font-semibold"
                    >
                        About Us
                    </h3>
                    <ul
                        class="text-muted-foreground space-y-2 text-sm"
                    >
                        <li>
                            <a
                                href="#"
                                class="hover:text-primary transition-colors"
                            >
                                Our
                                Story
                            </a>
                        </li>
                        <li>
                            <a
                                href="#"
                                class="hover:text-primary transition-colors"
                            >
                                Careers
                            </a>
                        </li>
                        <li>
                            <a
                                href="#"
                                class="hover:text-primary transition-colors"
                            >
                                Press
                            </a>
                        </li>
                        <li>
                            <a
                                href="#"
                                class="hover:text-primary transition-colors"
                            >
                                Blog
                            </a>
                        </li>
                    </ul>
                </div>

                <!-- Customer Service -->
                <div>
                    <h3
                        class="mb-4 text-lg font-semibold"
                    >
                        Customer Service
                    </h3>
                    <ul
                        class="text-muted-foreground space-y-2 text-sm"
                    >
                        <li>
                            <a
                                href="#"
                                class="hover:text-primary transition-colors"
                            >
                                Contact
                                Us
                            </a>
                        </li>
                        <li>
                            <a
                                href="#"
                                class="hover:text-primary transition-colors"
                            >
                                Shipping
                                Info
                            </a>
                        </li>
                        <li>
                            <a
                                href="#"
                                class="hover:text-primary transition-colors"
                            >
                                Returns
                                &
                                Exchanges
                            </a>
                        </li>
                        <li>
                            <a
                                href="#"
                                class="hover:text-primary transition-colors"
                            >
                                FAQ
                            </a>
                        </li>
                    </ul>
                </div>

                <!-- Privacy Policy -->
                <div>
                    <h3
                        class="mb-4 text-lg font-semibold"
                    >
                        Legal
                    </h3>
                    <ul
                        class="text-muted-foreground space-y-2 text-sm"
                    >
                        <li>
                            <a
                                href="#"
                                class="hover:text-primary transition-colors"
                            >
                                Privacy
                                Policy
                            </a>
                        </li>
                        <li>
                            <a
                                href="#"
                                class="hover:text-primary transition-colors"
                            >
                                Terms of
                                Service
                            </a>
                        </li>
                        <li>
                            <a
                                href="#"
                                class="hover:text-primary transition-colors"
                            >
                                Cookie
                                Policy
                            </a>
                        </li>
                        <li>
                            <a
                                href="#"
                                class="hover:text-primary transition-colors"
                            >
                                Accessibility
                            </a>
                        </li>
                    </ul>
                </div>

                <!-- Download Our App -->
                <div>
                    <h3
                        class="mb-4 text-lg font-semibold"
                    >
                        Download Our App
                    </h3>
                    <p
                        class="text-muted-foreground mb-4 text-sm"
                    >
                        Shop on the go
                        with our mobile
                        app
                    </p>
                    <div
                        class="flex flex-col gap-3"
                    >
                        <a
                            href="#"
                            class="border-border bg-background hover:bg-accent inline-flex items-center justify-center gap-2 rounded-lg border px-4 py-2 transition-colors"
                            aria-label="Download on App Store"
                        >
                            <svg
                                class="h-5 w-5"
                                fill="currentColor"
                                viewBox="0 0 24 24"
                                aria-hidden="true"
                            >
                                <path
                                    d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"
                                />
                            </svg>
                            <span
                                class="text-sm font-medium"
                            >
                                App
                                Store
                            </span>
                        </a>
                        <a
                            href="#"
                            class="border-border bg-background hover:bg-accent inline-flex items-center justify-center gap-2 rounded-lg border px-4 py-2 transition-colors"
                            aria-label="Get it on Google Play"
                        >
                            <svg
                                class="h-5 w-5"
                                fill="currentColor"
                                viewBox="0 0 24 24"
                                aria-hidden="true"
                            >
                                <path
                                    d="M3,20.5V3.5C3,2.91 3.34,2.39 3.84,2.15L13.69,12L3.84,21.85C3.34,21.6 3,21.09 3,20.5M16.81,15.12L6.05,21.34L14.54,12.85L16.81,15.12M20.16,10.81C20.5,11.08 20.75,11.5 20.75,12C20.75,12.5 20.5,12.92 20.16,13.19L16.81,15.12L14.54,12.85L20.16,10.81M6.05,2.66L16.81,8.88L14.54,11.15L6.05,2.66Z"
                                />
                            </svg>
                            <span
                                class="text-sm font-medium"
                            >
                                Google
                                Play
                            </span>
                        </a>
                    </div>
                </div>
            </div>

            <!-- Copyright -->
            <div
                class="text-muted-foreground mt-8 border-t pt-8 text-center text-sm"
            >
                <p>
                    &copy;
                    {{
                        new Date().getFullYear()
                    }}
                    ShopHub. All rights
                    reserved.
                </p>
            </div>
        </div>
    </footer>
</template>
